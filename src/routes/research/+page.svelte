
<div class="mb-4">
    <h1 class="text-2xl mb-1">Research Summary</h1>
    I do research both casually and at the University of Iowa. Primarily, I do research in fields of systems & networks, 
    but my projects may involve AI/deep learning.
</div>


<div class="mb-4">
<h1 class = "text-2xl mb-1">Experience</h1>
<h2 class="text-xl">Invidualized Research (CS:5990), University of Iowa</h2>
<h3 class="text-lg text-slate-800 mb-4">Professor Rishab Nithyanand, Jan 2024-May 2024</h3>

<h2 class="text-xl">Researcher, UI Hydroinformatics Laboratory</h2>
<h3 class="text-lg text-slate-800">Dr Omer Mermer, Oct 2023-Present</h3>
I am currently working as a research assistant with the UI Hydroinformatics Laboratory. My current project is 
a software for detecting algae blooms via remote sensing with Google Earth Engine.
</div>

<div class="mb-4">
<h1 class = "text-2xl mb-1">Publications</h1>    
To be updated later.
</div>
<h1 class="text-2xl">Other</h1>
I currently have an independent research project. The specs are not released, but it has been developed since 2022.
This project will likely have a few chunks/modules submitted to the SSTFI science fair for a chance of ISEF.